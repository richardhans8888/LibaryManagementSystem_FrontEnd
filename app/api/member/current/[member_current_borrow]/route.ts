...
const extractMemberId = (
  params: { member_current_borrow?: string | string[] } | undefined,
  url?: string
) => {
  const raw =
    params && "member_current_borrow" in params
      ? Array.isArray(params.member_current_borrow)
        ? params.member_current_borrow[0]
        : params.member_current_borrow
      : undefined;
...
//
// ========================================================
// GET /api/member/current/[member_current_borrow] → member's current borrowings
// ========================================================
export async function GET(
  req: Request,
  { params }: { params: { member_current_borrow: string | string[] } }
) {
  ...
  if (!rows) {
    return NextResponse.json(
      { success: false, error: "Failed to load current borrowings" },
      { status: 500 }
    );
  }
